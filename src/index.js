...
//   const ahmp = await authorsHashMap();
//   const comments = await getEndpoints('comments');
//   return comments.map(comment => ({
//     ...comments,
//     date: moment(comment.date).startOf('day').fromNow(),
//     sortableDate: moment(comment.date).format('YYYYMMDD'),
//     ts: moment(comment.date).unix(),
//     author: ahmp[comment.userId]
//   }));
// };
// const extendedArticles = async () => {
//   const extcmnts = await extendedComments();
//   const authoshmp = await authorsHashMap();
//   const articles = await getEndpoints('articles');
...
//       .sort((a, b) => (a.ts > b.ts ? 1 : -1)),
//     date: moment(article.date).format('Do MMMM YYYY'),
//     author: authoshmp[article.authorId],
//     tagsArr: article.tags,
//     tags: article.tags.map(slug => ({
//       slug,
//       title: tags[slug]
...
//   const extrtc = extendedArticles();
//   const authors = await getEndpoints('authors');
//   return authors.map(author => ({
//     ...author,
//     articles: extrtc.filter(({ authorId }) => authorId === author.id)
//   }));
// };

// const extendedTags = async () => {
//   const tags = await getEndpoints('tags');
//   return Object.keys(tags).map(slug => ({
//     slug,
//     title: tags[slug]
//   }));
// };
// // console.log(extendedArticles());

// module.exports = async () => {
//   extendedArticles, extendedAuthors;
// };
